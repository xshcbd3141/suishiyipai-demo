...
function saveConfig() {
  ...
  if (base) localStorage.setItem(SK.apiBase, base);
  if (key) localStorage.setItem(SK.apiKey, key);
  if (model) localStorage.setItem(SK.model, model);
}
...
function saveMeal() {
  ...
  localStorage.setItem(SK.meals, JSON.stringify(meals));
...
// ===== 用户档案 =====
function getProfile() {
  const p = JSON.parse(localStorage.getItem(SK.profile) || '{}');
...
function saveProfile() {
  const p = {
    ...
    age: parseInt($('pAge').value) || 25,
    height: parseFloat($('pHeight').value) || 170,
    weight: parseFloat($('pWeight').value) || 65,
    targetWeight: parseFloat($('pTargetWeight').value) || 60,
...
function renderProfilePage() {
  const raw = JSON.parse(localStorage.getItem(SK.profile) || '{}');
  $('pGender').value = raw.gender || 'male';
  $('pAge').value = raw.age || '';
  $('pHeight').value = raw.height || '';
  $('pWeight').value = raw.weight || '';
  $('pTargetWeight').value = raw.targetWeight || '';
  $('pActivity').value = raw.activity || '1.55';
...
function logWeight() {
  ...
  localStorage.setItem(SK.weightLog, JSON.stringify(deduped));
...
{"overall_score":85,"balance_score":80,"calorie_pct":92,"protein_pct":78,"alerts":["蛋白质不足","膳食纤维偏低"],"summary":"今日总结一句话","advice":"今日建议","tomorrow":"明日计划"}`;
  ...
  try {
    ...
    area.innerHTML = '<div style="color:#e74c3c;font-size:13px;">❌ 报告生成失败: ' + err.message.substring(0, 100) + '</div>';
  } finally {
...
function renderDigitalTwin() {
  const meals = JSON.parse(localStorage.getItem(SK.meals) || '{}');
  const wtLog = JSON.parse(localStorage.getItem(SK.weightLog) || '[]');
  const profile = getProfile();
...
【拒绝模板】当遇到应拒绝的问题时，简短回复："抱歉，我是营养健康助手，这方面给不了专业建议～需要我根据你的饮食记录，推荐一些适合当前状态的食物吗？"` },
    ...
    errDiv.innerHTML = '<div class="avatar">🤖</div><div class="bubble" style="color:#e74c3c;">抱歉，出了点问题：' + err.message.substring(0, 80) + '</div>';
...
// ===== 拖拽 & 粘贴 =====
const uz = $('uploadZone');
['dragover','dragenter'].forEach(e => uz.addEventListener(e, ev => { ev.preventDefault(); uz.style.borderColor = 'var(--green)'; }));
...
function renderDetailedDashboard() {
  ...
  // 热量构成解释
  const calTips = [];
  if (d.fatPct > 30) calTips.push('<span style="color:#c62828;">脂肪比偏高</span>');
  if (d.carbPct > 65) calTips.push('<span style="color:#e65100;">碳水比偏高</span>');
  if (d.proPct < 15) calTips.push('<span style="color:#c62828;">蛋白质比偏低</span>');
  if (calTips.length === 0) calTips.push('<span style="color:#07c160;">供能比例良好</span>');
  ...
  const mealCalRow = ['breakfast', 'lunch', 'dinner', 'snack']
    .map(mt => {
      ...
      const color = mt === 'dinner' && cal > d.avgCal * 0.45 ? '#e74c3c' : '#07c160';
  ...
  container.innerHTML = `
    ...
    <div class="macro-bar-card">
      ...
      <div class="macro-bar-row">
        <div class="macro-bar-seg" style="width:${d.proPct}%;background:#2e7d32;" title="蛋白质 ${d.proPct}%"></div>
        <div class="macro-bar-seg" style="width:${d.fatPct}%;background:#c62828;" title="脂肪 ${d.fatPct}%"></div>
        <div class="macro-bar-seg" style="width:${d.carbPct}%;background:#f9a825;border-radius:0 6px 6px 0;" title="碳水 ${d.carbPct}%"></div>
      </div>
...
async function generateMealRecommendation() {
  ...
  try {
    ...
    container.innerHTML = '<div style="color:#e74c3c;font-size:13px;text-align:center;padding:8px;">❌ ' + err.message.substring(0, 100) + '</div>';
  } finally {
...
function loadGameData() {
  let gd = JSON.parse(localStorage.getItem('ssyp_game') || '{}');
...
function saveGameData(gd) {
  localStorage.setItem('ssyp_game', JSON.stringify(gd));
}
...
function updateGameScore() {
  ...
  // 检测成就
  // 蔬菜覆盖率
  const allMeals = JSON.parse(localStorage.getItem(SK.meals) || '{}');
  ...
  container.innerHTML = `
    <div class="game-level-row">
      ...
      <div class="game-level-body">
        ...
        ` : '<div class="game-level-sub" style="color:#f9a825;">🏆 已满级！</div>'}
      </div>
...
{"summary":"周期总结(2-3句)","trend":"上升/下降/稳定","highlights":"亮点(1句话)","issues":"待改进(1句话)","advice":"下${period==='week'?'周':'月'}建议(具体可操作)"}`;
  ...
  try {
    ...
    area.innerHTML = `<div class="report-card" style="background:linear-gradient(135deg, #0ea5e9, #6366f1);">
    ...
    area.innerHTML = '<div style="color:#e74c3c;font-size:13px;">❌ ' + err.message.substring(0, 100) + '</div>';
  }
}
...
// ===== 初始化 =====
(function init() {
  const base = localStorage.getItem(SK.apiBase), key = localStorage.getItem(SK.apiKey), model = localStorage.getItem(SK.model);
